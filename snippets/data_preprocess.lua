  ...
  local split_fractions = {0.8, 0.2, 0 } -- train, validation and test splits.
  ...
  local all_classes = {} -- will contain all the rows from classes table.
  local all_logs = {} -- will contain all the rows from the logs table.
  local num_of_classes = cursor_classes:numrows()
  local num_of_examples = cursor_logs:numrows()
  local word_dim = 300
  local highest_num_of_words = 20 -- the highest number of words in a sentence. TODO: count it based on questions in the DB.

  local orig2rep_class_map = {} -- in case some classes have been ommited, e.g. if we have classes like: 1, 2, 10, 11, 12, etc.
  ...
  for i =1, num_of_classes do
      ...
      orig2rep_class_map[tonumber(row_table.id)] = i
  end
  ...
  print("\n\norig2rep Class Map:")
  for i, v in pairs(orig2rep_class_map) do
      print (i, v)
  end

  function getClassTensor(c)
 ...
 -- get all the logs(questions and their classes/answers)
  ...
  -- local all_data = torch.Tensor(num_of_examples, num_of_classes + word_dim * highest_num_of_words):fill(0)

  local all_data = {} -- this table will contain all the matrices of different length.
  -- so all_data[10] will return a matrix with all logs when question length is 10.
  ...
  for k, v in ipairs(all_logs) do
      ...
      local logTensor = torch.Tensor(1 + word_dim*i):fill(0) -- we need the first element to indicate which class the question belongs to.
      -- There is no need to make a one-hot encoding of y for NLLClassCriterion.

      logTensor[1] = orig2rep_class_map[q_class] -- set the first element to the correct class.

      local sentenceTensor  = torch.Tensor(i, word_dim):fill(0) -- TODO: need to remove sentenceTensor as we are not using this anymore.
      i = 0
      ...
      if not all_data[i] then
          all_data[i] = {}
          table.insert(all_data[i], logTensor)
      else
  ...
  -- Here we construct tensors(matrices) for each i in the all_data table. So that all questions of the same legnth are in 1 tensor.
  ...
  -- let's see what happens if we keep batch_size at 1 because we dont have much data to split it into many batches.
   ...
   local n_batches = num_of_examples

   if n_batches < 50 then
       print('WARNING: less than 50 batches in the data in total? Looks like very small dataset. You probably want to use smaller batch_size and/or seq_length.')
   end

       -- Now let's split the data (all_data table) into train, val, test sets.
  ...
  function next_batch(split_index)
      ...
      batch_ix[split_index] = batch_ix[split_index] + 1
      if batch_ix[split_index] > split_sizes[split_index] then
          batch_ix[split_index] = 1 -- cycle around to beginning when we reach the end of the set.
      end

      local ix = batch_ix[split_index]

      if split_index == 1 then
          return train_set[ix]:sub(2, train_set[ix]:size(1)), train_set[ix][1]
      end


      if split_index == 2 then
          return val_set[ix]:sub(2, val_set[ix]:size(1)), val_set[ix][1]
      end
  ...
  function spairs(t, order) -- takes in arguments t - a table to sort and order - function for sorting.
